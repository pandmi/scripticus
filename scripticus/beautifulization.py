...
def table_style(df,color,kpi):
        ...
        format_dict = {'total_spend':'{0:,.1f}', 'Latest Hour of Activity':'{0:,.0f}','Scheduled End Hour':'{0:,.0f}', 'total_revenue':'{0:,.1f}','NDC':'{0:,.2f}','LP':'{0:,.0f}', 'CPA_LP':'{0:,.2f}','CPC':'{0:,.2f}','CPA_Signup':'{0:,.2f}','CTR': '{:.2%}', 'CPA_NDC':'{0:,.1f}','CPA_DC':'{0:,.1f}','ROI': '{:.2f}','CPM': '{0:,.1f}', 'vCPM': '{0:,.1f}','CPC': '{0:,.1f}','CPA': '{0:,.1f}'}
...
#  underpacing strategies  

# Styling
# def frequency_min(data, color='red'):
#     '''
...
#     m3 = x['goal'] == 'vr'
#     df1 = pd.DataFrame('background-color: ', index=x.index, columns=x.columns)
#     df1['goal'] = np.where(((m0|m1|m2|m3)), 'background-color: {}'.format(r), df1['goal'])
#     return df1
...
# cm = sns.light_palette("blue", as_cmap=True)
# format_dict = {'Spend To Pace':'{0:,.1f}', 'latest_hour_of_delivery':'{0:,.0f}','end_hour':'{0:,.0f}','deal_mean':'{0:,.2f}', 'daily_spend':'{0:,.2f}', 'CPA_LP':'{0:,.2f}','CPC':'{0:,.2f}','CPA_Signup':'{0:,.2f}','CTR': '{:.2%}', 'CPA_NDC':'{0:,.1f}','CPA_DC':'{0:,.1f}','ROI': '{:.2f}','CPM': '{0:,.1f}', 'vCPM': '{0:,.1f}','CPC': '{0:,.1f}','CPA': '{0:,.1f}'}
# strundst = strategy_underpacing.style.format(format_dict).hide_index().\
# apply(reach,  subset=['goal']).\
# apply(reach_cpm, axis=None).\
# apply(vcr, axis=None).\
# apply(winrate, axis=None).\
...
#     def table_style(self, df,color,kpi):
#             cm = sns.light_palette(color, as_cmap=True)
#             format_dict = {'total_spend':'{0:,.1f}', 'Latest Hour of Activity':'{0:,.0f}','Scheduled End Hour':'{0:,.0f}', 'total_revenue':'{0:,.1f}','NDC':'{0:,.2f}','LP':'{0:,.0f}', 'CPA_LP':'{0:,.2f}','CPC':'{0:,.2f}','CPA_Signup':'{0:,.2f}','CTR': '{:.2%}', 'CPA_NDC':'{0:,.1f}','CPA_DC':'{0:,.1f}','ROI': '{:.2f}','CPM': '{0:,.1f}', 'vCPM': '{0:,.1f}','CPC': '{0:,.1f}','CPA': '{0:,.1f}'}
#             stdf = df.style.background_gradient(cmap=cm, subset=kpi).format(format_dict).hide_index()
#             return stdf