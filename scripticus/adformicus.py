...
def vzb_get_raw_stats(email, password, api_endpoint, start_date, end_date, dimensions,metrics,interval):
    access_token, advertiser_id= vzb_get_access_token(email, password)
    query_name=generate_query_name()
    query_id=vzb_create_specific_report_query(api_endpoint, access_token, advertiser_id,start_date,end_date, dimensions,metrics,interval, query_name)
    report_id= vzb_create_report(api_endpoint, access_token, advertiser_id, query_id)
...
def get_p161_raw_report(login, password, dimensions, metrics):
    timeframe="last_30_days"
    access_token, csrf_token = p161_authenticate(login, password)
    report_id = p161_create_report(access_token, csrf_token, dimensions, metrics,timeframe)
    result_id = p161_request_report_results(access_token, csrf_token, report_id)
...
def get_report_as_df(download_url: str, is_cloud_function: bool = False) -> pd.DataFrame:
    """
    Launches a Selenium browser to download a file from a URL and loads it into a pandas DataFrame.

    This function automatically handles temporary file creation and cleanup.

    Args:
        download_url: The direct URL that triggers the file download.
        is_cloud_function: Set to True when running in a cloud environment (like GCP).
                           This enables headless mode and uses packaged binaries.
    ...
    if is_cloud_function:
        print("Running in Cloud Function mode.")
        chrome_options.add_argument("--headless")
        chrome_options.add_argument("--no-sandbox")
        chrome_options.add_argument("--disable-dev-shm-usage")
        chrome_options.add_argument("--window-size=1920,1080")
        
        # In GCP, files must be saved to /tmp
        download_dir = "/tmp" 
        
        # Point to the binaries packaged with the function
        chrome_options.binary_location = "./bin/chrome"
        chromedriver_path = "./bin/chromedriver"
        service = ChromeService(executable_path=chromedriver_path)
    else:
        # Use webdriver-manager for easy local execution
    ...
    # --- 2. Download the file and clean up ---
    try:
        ...
        for _ in range(30): # 30-second timeout
            ...
            if new_files and not any(f.endswith('.crdownload') for f in new_files):
                ...
                # --- The main goal is achieved: we have the DataFrame ---
                return df
            time.sleep(1)
        
        raise TimeoutError("File download timed out after 30 seconds.")
...
def pivoted_brand_daily_report(client, current_date, networks=None, exclude=None):
    ...
    if networks:
        networks_str = ",".join(f"'{n}'" for n in networks)
        query = f"""
        SELECT *
        FROM `dwh-landing-v1.paid_media_reports_eu_west_2.looker_daily_brand_performance`
        WHERE date >= '{start_date}'
          AND date <= '{end_date}'
          AND network IN ({networks_str})
        """
    elif exclude:
        exclude_str = ",".join(f"'{n}'" for n in exclude)
        query = f"""
        SELECT *
        FROM `dwh-landing-v1.paid_media_reports_eu_west_2.looker_daily_brand_performance`
        WHERE date >= '{start_date}'
          AND date <= '{end_date}'
          AND network NOT IN ({exclude_str})
        """
    else:
        query = f"""
        SELECT *
    ...
    if networks:
        metric_rename_map = {
            'total_spend': 'Spend_fixed_networks',
            'ROAS_35': 'ROAS_35_fixed_networks',
            'FTD_CPA':'FTD_CPA_fixed_networks'}
        final_df['metric'] = final_df['metric'].replace(metric_rename_map) 
        final_df=final_df[final_df['metric'].isin(['Spend_fixed_networks', 'ROAS_35_fixed_networks', 'FTD_CPA_fixed_networks'])]

    elif exclude:
        metric_rename_map = {
            'total_spend': 'Spend_dsp',
            'ROAS_35': 'ROAS_35_dsp', 
            'FTD_CPA': 'FTD_CPA_dsp'
        }
        final_df['metric'] = final_df['metric'].replace(metric_rename_map)
        final_df=final_df[final_df['metric'].isin(['Spend_dsp', 'ROAS_35_dsp', 'FTD_CPA_dsp'])]
...
def full_pivoted_brand_daily_report(client, current_date, networks):
    ...
    df_fin = pd.concat([df,df_net,df_excl], ignore_index=True)
    metric_order_list = [
        'Impressions', 'Clicks', 'Spend','Spend_dsp','Spend_fixed_networks', 'Revenue',
        'Registration', 'WalletConnected', 'Reccuring Deposits', 'Reccuring Deposits Sales', 'FTD',
        'FTD Sales', 'Install', 'SignUp', 'CPM', 'CTR', 'CPC',
        'Registration_CPA', 'FTD_CPA','FTD_CPA_dsp','FTD_CPA_fixed_networks', 'Deposit_CPA', 'ROAS', 'ROAS_35','ROAS_35_dsp','ROAS_35_fixed_networks',
        'ROI_35'
    ]
    ...
    # Add the column to the DataFrame
    df_fin['metric_order'] = df_fin['metric'].map(metric_order_map)
    df_fin = df_fin.sort_values(['date', 'Vertical', 'Brand', 'metric_order']).reset_index(drop=True)
